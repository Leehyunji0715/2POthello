...
public class ServerBoard extends JFrame{
	...
	public class GameRuleSetting{
		...
		public void checkChangeStone() {
			...
			if((ps.user==1&&buttons[pos_x-1][pos_y].value==2)||ps.user==2&&buttons[pos_x-1][pos_y].value==1) {
				...
				{
					if(ix==-1)	break;
					if(buttons[pos_x-1][pos_y].value==0)
						break;
					if((buttons[pos_x-1][pos_y].value==2 && buttons[ix][pos_y].value==1 || (buttons[pos_x-1][pos_y].value==1 && buttons[ix][pos_y].value==2)))	
					{
						...
						{	
							if(ps.user==1) {
								buttons[jx][pos_y].setIcon(ps.p1);
								buttons[jx][pos_y].value = 1;
							}
							else if(ps.user==2) {
								buttons[jx][pos_y].setIcon(ps.p2);
								buttons[jx][pos_y].value = 2;
							}
						}
						break;
						}
				...
				if ((ps.user == 1 && buttons[pos_x-1][pos_y-1].value == 2) || (ps.user == 2 && buttons[pos_x-1][pos_y-1].value == 1))	// 상 왼쪽 방향
				{
					for(int ix=pos_x-1,iy=pos_y-1; ix>=0 || iy>=0; ix-- , iy--)	// 상 왼쪽 방향으로 돌 조사
					{
						...
						if((buttons[pos_x-1][pos_y-1].value==2 && buttons[ix][iy].value==1) || (buttons[pos_x-1][pos_y-1].value==1 && buttons[ix][iy].value==2))	// 반대속성의 돌이 나오면
						{
							...
							{	
								if(ps.user==1) {
									buttons[jx][jy].setIcon(ps.p1);
									buttons[jx][jy].value = 1;
								}
								else if(ps.user==2) {
									buttons[jx][jy].setIcon(ps.p2);
									buttons[jx][jy].value = 2;
								}
							}
							break;
						}
				...
				if ((ps.user == 1 && buttons[pos_x][pos_y-1].value==2) || (ps.user == 2 && buttons[pos_x][pos_y-1].value==1))	// 중간 왼쪽 방향
				{
					for(int iy=pos_y-1; iy>=0; iy--)	// 왼쪽 방향으로 돌 조사
					{
						...
						if((buttons[pos_x][pos_y-1].value==2 && buttons[pos_x][iy].value==1) || (buttons[pos_x][pos_y-1].value==1 && buttons[pos_x][iy].value==2))	// 반대속성의 돌이 나오면
						{
							...
							{	
								if(ps.user==1) {
									buttons[pos_x][jy].setIcon(ps.p1);
									buttons[pos_x][jy].value = 1;
								}
								else if(ps.user==2) {
									buttons[pos_x][jy].setIcon(ps.p2);
									buttons[pos_x][jy].value = 2;
								}
							}
							break;
						}
	...
	public class ScoreBoardin extends JFrame{
		...
		public void changeNum() {
			p1_10digits_score = (int)(p1_score/10);
			p1_1digit_score = (p1_score%10);
			
			p2_10digits_score = (int)(p2_score/10);
			p2_1digit_score = (p2_score%10);
		...
		public void result() {
			if(p1_score + p2_score == 64) {
				...
				if(p1_score>p2_score) {
					...
					p1.setIcon(PlayerSetting.p1_win);
					p1.setBounds(50, 0, 100, 100);
					JOptionPane.showMessageDialog(null,"노랑이 승리 !!!!!!","두구두구",JOptionPane.PLAIN_MESSAGE);
				}
				else if(p1_score<p2_score) {
					System.out.println("******게임 끝******");
					p2.setIcon(PlayerSetting.p2_win);
					p2.setBounds(200, 0, 100, 100);
					JOptionPane.showMessageDialog(null,"검정이 승리 !!!!!!","두구두구",JOptionPane.PLAIN_MESSAGE);
					}
				else if(p1_score==p2_score) {
					System.out.println("******게임 끝******");
					p1.setIcon(PlayerSetting.p1_win);
					...
					p1.setBounds(50, 0, 100, 100);
					p2.setBounds(200, 0, 100, 100);
					JOptionPane.showMessageDialog(null,"무승부 !!!!!!","두구두구",JOptionPane.PLAIN_MESSAGE);
				}
			}
		}
		public void panelCreator(){
	        ServerChatPanel serverPanel=new ServerChatPanel(server);
	        serverPanel.setLayout(null);
	        serverPanel.setLocation(10,208);
	        add(serverPanel);
	    }
	}
